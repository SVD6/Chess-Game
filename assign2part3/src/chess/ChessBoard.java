...
public class ChessBoard {
    ...
    public String toTakenString() {
        String result = "";
            for (int i = 0; i < takenpieces.size() && takenpieces.get(i).getColour() == ChessColour.WHITE; i ++) {
                result += takenpieces.get(i);
            }
            result += ",";
            for (int i = 0; i < takenpieces.size() && takenpieces.get(i).getColour() == ChessColour.BLACK; i ++) {
                result += takenpieces.get(i).getShortName();
            }
        return result;
    }    
}
